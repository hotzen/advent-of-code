...
fun main() {
    val div2 = Packet.Ls(mutableListOf(Packet.Ls(mutableListOf(Packet.Num(2)))))
    val div6 = Packet.Ls(mutableListOf(Packet.Ls(mutableListOf(Packet.Num(6)))))

    val packets = getResourceAsFile("day13.txt").useLines { lines ->
        lines.chunked(3).flatMapIndexed { idx, chunk ->
            val p1 = Parser.parse(chunk[0])
            val p2 = Parser.parse(chunk[1])
            listOf(p1, p2)
        }.toList()
    } + listOf(div2, div6)

    val sorted = packets.sorted()
    val div2Pos = sorted.indexOf(div2) + 1
    val div6Pos = sorted.indexOf(div6) + 1
    println(div2Pos * div6Pos)
}
